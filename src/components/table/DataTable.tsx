...
// import { convertDate } from "utils/index.tsx";
// import ActionButton from "./ActionButton";
// import FilterButton from "./FilterButton";
// import useLocalStorage from 'hooks/useLocalStrorage.tsx';
...
const DataTableComponent = ({ data }: { data: USER[] }) => {
  ...
  const determineStatus = (index: number) => {
    ...
    if (Math.sqrt(num) % 1 === 0) type = "pending";
    if (Math.cbrt(num) % 1 === 0) type = "inactive";
  ...
  const handleFilterShow = () => setOpenFilter((prev) => !prev);
        ...
        <tbody className="table-body">
          {currentTableData.length > 0 ? (
            currentTableData.map((item, index) => (
              <tr key={item.id}>
                <td data-label={headers[0]}>{item.organization}</td>
                <td data-label={headers[1]}>{item.username}</td>
                <td data-label={headers[2]}>{item.email}</td>
                <td data-label={headers[3]}>{item.phoneNumber}</td>
                <td data-label={headers[4]}>
                  {convertDate(item.dateJoined)}
                </td>
                <td data-label={headers[5]}>{determineStatus(index)}</td>
                <td>
                  <ActionButton
                    id={item.id}
                    saveSelectedUserIdStorage={() =>
